...
This module provides the menu UI for selecting how transcribed text should be
output to the user after successful transcription. It supports two modes:

1. Copy to Clipboard: Simply copies the transcribed text to the system clipboard
2. Paste at Cursor: Copies to clipboard and then simulates Command+V to paste at the
   current cursor position (requires Accessibility permission)

The user's selection is persisted in the application configuration and used by
the main application to determine how to handle transcription results.
"""
...
class OutputMenu(BaseMenuComponent):
    ...
    This component manages the output action menu items and handles switching
    between output modes (clipboard/paste at cursor). It provides a submenu with
    radio-button style options that allow the user to select how transcribed text
    should be delivered.
    
    The "Paste at Cursor" option requires Accessibility permission to function
    correctly, as it uses Quartz keyboard events to simulate Command+V after
    copying the text to the clipboard.
    
    Attributes:
        output_action_callback: Callback to invoke when output action is changed
        output_menu_item: The main menu item that contains the output options
        copy_menu_item: Menu item for the "Copy to Clipboard" option
        paste_menu_item: Menu item for the "Paste at Cursor" option
    """
