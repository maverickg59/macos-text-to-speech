...
This module provides the menu UI for selecting different Whisper model sizes.
It handles both the UI presentation and the model switching logic when the user
selects a different model from the menu.
"""
...
class ModelMenu(BaseMenuComponent):
    ...
    handles model switching. It provides a submenu with all available Whisper models
    and manages the checkmarks to indicate the currently selected model.
    
    When a model is selected, it updates the configuration and invokes a callback
    to notify the application that the model has changed.
    
    Attributes:
        model_callback: Function called when model is changed with the new model name as argument
        model_menu_item: The main menu item that contains the model submenu
        model_submenu_items: Dictionary mapping model names to their menu items
    """
    ...
    def get_available_models(self) -> List[str]:
        ...
        """
        return AVAILABLE_WHISPER_MODELS.copy()
