...
from src.av_audio import create_input_stream, query_devices
...
from src.constants import (
    ...
    DEFAULT_MIN_RECORDING_TIME,
    DEFAULT_SILENCE_THRESHOLD_DB, DEFAULT_SILENCE_DURATION
)
...
class AudioRecorder:
    ...
    Note:
        This class explicitly forces CPU mode for faster-whisper on all platforms,
        as MPS acceleration is not supported on Apple Silicon Macs. This ensures
        consistent behavior across different Mac hardware.
    """
    # Use centralized audio state constants
    STATE_IDLE = AUDIO_STATE_IDLE
    STATE_WAITING_FOR_SPEECH = AUDIO_STATE_WAITING_FOR_SPEECH
    STATE_SPEECH_DETECTED = AUDIO_STATE_SPEECH_DETECTED

    def __init__(self, config, transcription_callback=None):
        """Initialize the AudioRecorder with the given configuration.
        
        This class handles both audio recording and transcription using faster-whisper.
        Note that we explicitly force CPU mode for the model since faster-whisper 
        does not support MPS acceleration on Apple Silicon Macs.
        
        Args:
            config: The application configuration object containing audio and transcription settings
            transcription_callback: Optional callback function that will be called with transcription results
        """
        ...
        self.device = WHISPER_DEVICE_CPU  # Explicitly use CPU as faster-whisper doesn't support MPS
        self.compute_type = WHISPER_COMPUTE_INT8
        ...
        self.min_recording_time = DEFAULT_MIN_RECORDING_TIME
