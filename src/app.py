"""Main application module for Ottotone speech-to-text menubar application.

This module serves as the central orchestrator for the Ottotone application,
integrating all the core components (audio recording, transcription, permissions,
hotkeys, and menu system) into a cohesive menubar application.

It provides the OttotoneApp class which is responsible for:
1. Setting up and managing the menubar interface using rumps
2. Initializing and coordinating all application components
3. Handling user interactions and menu actions
4. Managing the application lifecycle and resource cleanup
5. Implementing the core recording and transcription workflow

The application architecture follows a modular design with clear separation of
concerns between different functional components, allowing for maintainable
and extensible code.
"""
...
from src.constants import (
    # App constants
    APP_NAME, APP_VERSION, DEFAULT_TITLE, MENU_ICON_FILE, APP_ICON_FILE, 
    RESOURCES_DIR, PLATFORM_DARWIN, DEV_MODE,
    
    # Menu constants
    MENU_RECORD, MENU_STOP, MENU_CHECK_PERMISSIONS, MENU_QUIT,
    
    # Output constants
    OUTPUT_ACTION_CLIPBOARD, OUTPUT_ACTION_PASTE_AT_CURSOR,
    ...
    # Audio constants
    AVAILABLE_WHISPER_MODELS, TRANSCRIPTION_LANGUAGES
)
...
from src.paste import paste_text_at_cursor
from src.menu import MenuManager, ModelMenu, OutputMenu, SettingsMenu, PermissionsMenu

# Available Whisper models are imported from src.config.constants as AVAILABLE_WHISPER_MODELS

# --- Logging Setup ---
# Note: In production, this should be level=logging.INFO
...
class OttotoneApp(rumps.App):
    """Main application class for the Ottotone menubar application.
    
    This class serves as the central coordinator for the entire application,
    integrating all core components and managing the application lifecycle.
    It inherits from rumps.App to provide the menubar functionality.
    
    Key responsibilities:
    - Managing the application configuration and settings
    - Checking and handling system permissions
    - Setting up the audio recording and transcription pipeline
    - Configuring global hotkeys for recording activation
    - Building and managing the menubar interface
    - Handling user interactions with the menu items
    - Ensuring proper cleanup of resources on application exit
    
    Note:
        The application explicitly forces CPU mode for faster-whisper on all Macs
        as MPS acceleration is not supported on Apple Silicon hardware.
    """
    def __init__(self):
    ...
    def _process_transcription_complete(self, data):
        ...
        if output_action == OUTPUT_ACTION_CLIPBOARD:
            ...
            if self.permissions_manager.check_accessibility_permission(prompt_if_needed=False):
                ...
                success = paste_text_at_cursor(final_text)
                if success:
                    logger.info("Successfully pasted text at cursor.")
                    self._send_notification(APP_NAME, "Pasted at Cursor", final_text, priority="low")
                else:
                    logger.warning("Paste at cursor operation failed.")
                    self._send_notification(APP_NAME, "Paste Failed", "Could not paste at cursor location", priority="high")
            else:
    ...
    def _setup_audio_recorder(self):
        ...
        default_model = AVAILABLE_WHISPER_MODELS[0] if AVAILABLE_WHISPER_MODELS else "tiny"

        if current_selected_model not in AVAILABLE_WHISPER_MODELS:
            logger.warning(
                f"Loaded model '{current_selected_model}' from settings is not in available models {AVAILABLE_WHISPER_MODELS}. "
